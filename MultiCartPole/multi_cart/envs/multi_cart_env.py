...
class MultiCartPoleEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.cartpoles = [
            SingleCart(offset=i * MultiCartPoleEnv.CARTDIST)
            for i in range(MultiCartPoleEnv.CARTPOLES)
        ]
        ...
        # define spaces
        self.action_space = spaces.MultiBinary(MultiCartPoleEnv.CARTPOLES)
        self.observation_space = spaces.Tuple(tuple([
            spaces.Box(-high, high, dtype=np.float32)
            for _ in range(MultiCartPoleEnv.CARTPOLES)
        ]))
...
class SingleCart():
    ...
    def is_done(self):
        ...
        return bool(
            x - self.offset < -self.x_threshold
            or x - self.offset > self.x_threshold
            or theta < -self.theta_threshold_radians
            or theta > self.theta_threshold_radians
        )

    def render(self, viewer, screen_width, init=False, mode='human'):
        world_width = self.x_threshold * 2 + (MultiCartPoleEnv.CARTPOLES - 1) * MultiCartPoleEnv.CARTDIST
        ...
        cartx -= screen_width / 2 #- self.x_threshold * scale)
        cartx += self.x_threshold * scale
